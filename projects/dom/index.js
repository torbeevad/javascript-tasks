...
function prepend(what, where) {
  const element = what;
  const element2 = where;

  element2.insertBefore(element, element2.firstChild);
}
...
function deleteTextNodesRecursive(where) {
  for (const node of Array.from(where.childNodes)) {
    if (node.nodeType === Element.TEXT_NODE) {
      node.remove();
    } else if (node.nodeType === Element.ELEMENT_NODE && node.hasChildNodes()) {
...
/*
 ...
 8.2: При вызове fn необходимо передавать ей в качестве аргумента объект с двумя свойствами:
   ...
   - elements: массив из удаленных или добавленных элементов (в зависимости от события)
